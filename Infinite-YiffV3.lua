...
if getgenv().IY_LOADED and getgenv().IY_YIFF_GUI then
    pcall(function() getgenv().IY_YIFF_GUI:Destroy() end)
end
...
Utilities.Players = Players
Utilities.TweenService = TweenService
Utilities.RunService = RunService
Utilities.CoreGui = CoreGui
Utilities.UserInputService = UserInputService
Utilities.StarterGui = StarterGui
Utilities.HttpService = HttpService
Utilities.LocalPlayer = Players.LocalPlayer

function Utilities.randomString()
...
function Utilities.getHiddenGui()
...
function Utilities.protectGui(gui)
	...
	gui.Parent = Utilities.getHiddenGui()
end

function Utilities.tween(object, style, direction, time, properties)
...
function Utilities.GetRoot(char)
...
Modules['LoadingScreen'] = (function()
local LoadingScreen = {}
...
function LoadingScreen.Show()
	...
	ScreenGui.Name = Utilities.randomString()
	ScreenGui.ResetOnSpawn = false
	Utilities.protectGui(ScreenGui)

	Background = Instance.new("Frame")
	Background.Name = "LoadBg"
	Background.Parent = ScreenGui
	Background.Size = UDim2.new(1, 0, 1, 0)
	Background.Position = UDim2.new(0, 0, 0, 0)
	Background.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
	Background.BackgroundTransparency = 1 -- We will fade it slightly if desired, or keep it totally transparent just showing logo
	Background.BorderSizePixel = 0
	
	LogoIcon = Instance.new("ImageLabel")
	LogoIcon.Name = "Logo"
	LogoIcon.Parent = Background
	LogoIcon.AnchorPoint = Vector2.new(0.5, 0.5)
	LogoIcon.Position = UDim2.new(0.5, 0, 1.2, 0) -- Starts at bottom
	LogoIcon.Size = UDim2.new(0, 80, 0, 80)
	LogoIcon.BackgroundTransparency = 1
	LogoIcon.Image = "rbxassetid://1352543873" -- Default Infinite Yield logo
	LogoIcon.ImageTransparency = 0

	-- Slide up to middle
	Utilities.tween(LogoIcon, "Quart", "Out", 1.2, {Position = UDim2.new(0.5, 0, 0.5, 0)})
	
	task.wait(0.5) -- wait slightly before spinning

	-- Spin forever until IsDone
	local spinInfo = TweenInfo.new(1.5, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1, false)
	SpinTween = TweenService:Create(LogoIcon, spinInfo, {Rotation = 360})
	SpinTween:Play()
end
...
function LoadingScreen.Hide()
	...
	-- Tween back to normal rotation and fade out
	Utilities.tween(LogoIcon, "Quart", "Out", 0.5, {Rotation = 0, ImageTransparency = 1})
	task.wait(0.5)
...
Modules['Visualizer'] = (function()
local Visualizer = {}

local Utilities = require("Utilities")
local TweenService = Utilities.TweenService
local UserInputService = Utilities.UserInputService
local RunService = Utilities.RunService
...
local Options = {
	...
	BackgroundColor = Color3.fromRGB(36, 36, 37),
...
local tabHeld = false
local tabHoldStart = 0
local CurrentQuery = ""

function Visualizer.Init(Core)
	MainGui = Instance.new("ScreenGui")
	MainGui.Name = Utilities.randomString()
	MainGui.ResetOnSpawn = false
	Utilities.protectGui(MainGui)
    
    if setreadonly then setreadonly(getgenv(), false) end
    getgenv().IY_YIFF_GUI = MainGui

	Background = Instance.new("Frame")
	Background.Name = "VisualizerBg"
	Background.Parent = MainGui
	Background.Size = Options.Size
	Background.Position = UDim2.new(0.5, -200, 1, 50)
	Background.BackgroundColor3 = Options.BackgroundColor
	Background.BackgroundTransparency = Options.Transparency
	Background.BorderSizePixel = 0
	Background.ClipsDescendants = false

	local UICorner = Instance.new("UICorner")
	UICorner.CornerRadius = Options.CornerRadius
	...
	InputField.Parent = Background
	InputField.Size = UDim2.new(1, -20, 1, 0)
	InputField.Position = UDim2.new(0, 10, 0, 0)
	InputField.BackgroundTransparency = 1
	InputField.Font = Options.Font
	InputField.TextSize = 18
	InputField.TextColor3 = Options.TextColor
	InputField.PlaceholderText = "Type a command..."
	InputField.PlaceholderColor3 = Color3.fromRGB(150, 150, 150)
	InputField.TextXAlignment = Enum.TextXAlignment.Left
	InputField.ClearTextOnFocus = false
	InputField.Text = ""

	SuggestionList = Instance.new("Frame")
	SuggestionList.Name = "Suggestions"
	SuggestionList.Parent = Background
	SuggestionList.Size = UDim2.new(1, 0, 0, 0)
	SuggestionList.Position = UDim2.new(0, 0, 0, -5)
	SuggestionList.AnchorPoint = Vector2.new(0, 1)
	SuggestionList.BackgroundTransparency = 1
	SuggestionList.ClipsDescendants = true

	UIListLayout = Instance.new("UIListLayout")
	UIListLayout.Parent = SuggestionList
	...
	InputField:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        if string.find(text, "\t") then
            InputField.Text = string.gsub(text, "\t", "")
            return 
        end
		if text ~= "" then
            CurrentQuery = text
			Visualizer.UpdateSuggestions(Core, text)
		else
            CurrentQuery = ""
			Visualizer.ClearSuggestions()
		end
	end)

	InputField.FocusLost:Connect(function(enterPressed)
		if enterPressed then
			local result = InputField.Text
			Visualizer.Toggle(false)
			if result ~= "" then
                task.spawn(function()
				    Core.ExecuteCommand(result)
                end)
			end
		end
	...
	UserInputService.InputBegan:Connect(function(input, gameProcessed)
		if input.KeyCode == Enum.KeyCode.Semicolon then
            if IsOpen then
                ...
                if not gameProcessed and not getgenv().IY_LEGACY_MODE then
                    task.wait()
                    Visualizer.Toggle(true)
                end
            end
        elseif input.KeyCode == Enum.KeyCode.Tab then
            if IsOpen and InputField:IsFocused() then
                tabHeld = true
                tabHoldStart = tick()
            end
		end
    ...
    UserInputService.InputEnded:Connect(function(input, gameProcessed)
        if input.KeyCode == Enum.KeyCode.Tab then
            if tabHeld and tick() - tabHoldStart < 0.3 then
                Visualizer.CycleSelection()
            end
            tabHeld = false
        end
    end)
    ...
    if UserInputService.TouchEnabled then
        ...
        mobileToggleBtn.Position = UDim2.new(1, -60, 1, -60)
        mobileToggleBtn.BackgroundColor3 = Options.BackgroundColor
        mobileToggleBtn.BackgroundTransparency = Options.Transparency
        mobileToggleBtn.Text = ";"
        mobileToggleBtn.TextColor3 = Options.TextColor
        ...
        mobileToggleBtn.MouseButton1Click:Connect(function()
            if not getgenv().IY_LEGACY_MODE then
                Visualizer.Toggle(not IsOpen)
            end
        end)
    end
end
...
RunService.RenderStepped:Connect(function()
    if tabHeld and tick() - tabHoldStart >= 0.3 then
        Visualizer.AutofillSelection()
        tabHeld = false
    end
end)

function Visualizer.Toggle(state)
	IsOpen = state
	if state then
		Utilities.tween(Background, "Quart", "Out", 0.4, {Position = Options.Position, BackgroundTransparency = Options.Transparency})
		InputField.Text = ""
		Visualizer.ClearSuggestions()
		InputField:CaptureFocus()
	else
        Visualizer.ClearSuggestions()
		Utilities.tween(Background, "Quart", "In", 0.3, {Position = UDim2.new(0.5, -200, 1, 50), BackgroundTransparency = 1})
...
function Visualizer.CycleSelection()
    if #CurrentMatches > 0 then
        SelectedIndex = SelectedIndex + 1
        if SelectedIndex > #CurrentMatches then
            SelectedIndex = 1
        end
        Visualizer.RenderSuggestions()
    end
end

function Visualizer.AutofillSelection()
    if CurrentMatches[SelectedIndex] then
        local nameStr = CurrentMatches[SelectedIndex].Name
        
        nameStr = string.gsub(nameStr, "%[.-%]", "")
        nameStr = string.gsub(nameStr, "<.->", "")
        
        local aliases = {}
        for alias in string.gmatch(nameStr, "[^/]+") do
            alias = string.match(alias, "^%s*(.-)%s*$")
        ...
        if #aliases > 0 then
            local chosen = aliases[math.random(1, #aliases)]
            for _, alias in ipairs(aliases) do
                if string.find(string.lower(alias), string.lower(CurrentQuery), 1, true) then
                    chosen = alias
                    break
                end
            end
...
function Visualizer.ClearSuggestions()
	for _, child in pairs(SuggestionList:GetChildren()) do
		if child:IsA("Frame") or child:IsA("TextButton") then
			child:Destroy()
		end
	end
	Utilities.tween(SuggestionList, "Quart", "Out", 0.2, {Size = UDim2.new(1, 0, 0, 0)})
end

function Visualizer.UpdateSuggestions(Core, query)
	CurrentMatches = Core.GetCommandSuggestions(query)
    SelectedIndex = 1
    Visualizer.RenderSuggestions()
end

function Visualizer.RenderSuggestions()
    Visualizer.ClearSuggestions()
    ...
    local startIndex = 1
    if SelectedIndex > maxSuggestions then
        startIndex = SelectedIndex - maxSuggestions + 1
    end

	for i = startIndex, math.min(#CurrentMatches, startIndex + maxSuggestions - 1) do
        local cmdInfo = CurrentMatches[i]
		local sFrame = Instance.new("TextButton")
		sFrame.Size = UDim2.new(1, 0, 0, itemHeight)
        sFrame.Text = ""
        sFrame.AutoButtonColor = false
		sFrame.BackgroundTransparency = 0.2
		sFrame.BorderSizePixel = 0

        if i == SelectedIndex then
            sFrame.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
            local indicator = Instance.new("Frame")
            indicator.Size = UDim2.new(0, 3, 1, 0)
            indicator.BackgroundColor3 = Options.AccentColor
            indicator.BorderSizePixel = 0
            indicator.Parent = sFrame
            local icorner = Instance.new("UICorner")
            icorner.CornerRadius = UDim.new(0, 6)
            icorner.Parent = indicator
        else
            sFrame.BackgroundColor3 = Color3.fromRGB(46, 46, 47)
        end
		...
		sText.Font = Options.Font
		sText.TextSize = 16
		sText.RichText = true
		sText.TextColor3 = Color3.fromRGB(180, 180, 180)
		sText.TextXAlignment = Enum.TextXAlignment.Left
		
		local nStr = cmdInfo.Name
        local queryStr = CurrentQuery
		local matchStart, matchEnd = string.find(string.lower(nStr), string.lower(queryStr), 1, true)
		if matchStart then
			local before = string.sub(nStr, 1, matchStart - 1)
			local matched = string.sub(nStr, matchStart, matchEnd)
			local after = string.sub(nStr, matchEnd + 1)
			sText.Text = "<font color='rgb(180,180,180)'>" .. before .. "</font><b><font color='rgb(255,255,255)'>" .. matched .. "</font></b><font color='rgb(180,180,180)'>" .. after .. "</font> - <font color='rgb(120,120,120)'>" .. cmdInfo.Description .. "</font>"
		else
			sText.Text = "<b><font color='rgb(255,255,255)'>" .. nStr .. "</font></b> - <font color='rgb(120,120,120)'>" .. cmdInfo.Description .. "</font>"
		end
		
        sFrame.MouseButton1Down:Connect(function()
            SelectedIndex = i
            Visualizer.AutofillSelection()
        end)

		sFrame.Parent = SuggestionList
		count = count + 1
		totalHeight = totalHeight + itemHeight + UIListLayout.Padding.Offset
	end

	if count > 0 then
		totalHeight = totalHeight - UIListLayout.Padding.Offset
		Utilities.tween(SuggestionList, "Quart", "Out", 0.3, {Size = UDim2.new(1, 0, 0, totalHeight)})
	end
end

local activeNotifications = {}

function Visualizer.Notify(title, text, time)
    time = time or 5
    local notifyGui = MainGui:FindFirstChild("Notifications")
    if not notifyGui then
        notifyGui = Instance.new("Frame")
        notifyGui.Name = "Notifications"
        notifyGui.Parent = MainGui
        notifyGui.Size = UDim2.new(0, 300, 1, 0)
        notifyGui.Position = UDim2.new(1, -320, 0, 0)
    ...
    notifFrame.BackgroundColor3 = Options.BackgroundColor
    notifFrame.BackgroundTransparency = Options.Transparency
    notifFrame.BorderSizePixel = 0
    notifFrame.Position = UDim2.new(1, 300, 0, 0) -- starts offscreen
    ...
    dLabel.Size = UDim2.new(1, -20, 1, -35)
    ...
    pinBtn.MouseButton1Click:Connect(function()
        ...
        if isPinned then
            pinBtn.TextColor3 = Options.AccentColor
        else
            pinBtn.TextColor3 = Color3.fromRGB(150, 150, 150)
        end
    end)
    ...
    local function closeNotif()
        ...
        isClosed = true
        local outTween = Utilities.tween(notifFrame, "Quart", "In", 0.5, {Position = UDim2.new(1, 300, 0, 0), BackgroundTransparency = 1})
    ...
    Utilities.tween(notifFrame, "Quart", "Out", 0.5, {Position = UDim2.new(0, 0, 0, 0)})
...
local CloseBtn
    ...
    local RuntimeCore = nil

function CmdsWindow.Init(Core)
    RuntimeCore = Core
    MainGui = getgenv().IY_YIFF_GUI
    
    Background = Instance.new("Frame")
    Background.Name = "CmdsWindow"
    Background.Parent = MainGui
    Background.Size = UDim2.new(0, 450, 0, 350)
    Background.Position = UDim2.new(0.5, -225, 0.5, -175)
    Background.BackgroundColor3 = Visualizer.Options.BackgroundColor
    Background.BackgroundTransparency = Visualizer.Options.Transparency
    Background.BorderSizePixel = 0
    Background.Visible = false
    Background.ClipsDescendants = true
    
    local UICorner = Instance.new("UICorner")
    UICorner.CornerRadius = Visualizer.Options.CornerRadius
    UICorner.Parent = Background
    ...
    TopCorner.CornerRadius = Visualizer.Options.CornerRadius
    ...
    Title.TextSize = 18
    Title.TextColor3 = Visualizer.Options.TextColor
...
function CmdsWindow.Toggle(state)
    if state == nil then
        state = not Background.Visible
    end
    if state then
        if RuntimeCore then
            local allCmds = RuntimeCore.GetAllCommands()
            if not allCmds then allCmds = {} end
            local titleObj = TopBar:FindFirstChild("Title")
            if titleObj then
                titleObj.Text = "CMDs - " .. tostring(#allCmds) .. " Commands"
            end
            ...
            for i, cmd in ipairs(allCmds) do
                local cFrame = Instance.new("Frame")
                cFrame.Size = UDim2.new(1, -10, 0, 30)
                cFrame.BackgroundColor3 = Color3.fromRGB(46, 46, 47)
                cFrame.BackgroundTransparency = 0.3
                cFrame.BorderSizePixel = 0
                cFrame.Parent = ListFrame
                ...
                cText.Parent = cFrame
                cText.Size = UDim2.new(1, -10, 1, 0)
                cText.Position = UDim2.new(0, 10, 0, 0)
                cText.BackgroundTransparency = 1
                cText.Font = Visualizer.Options.Font
                cText.TextSize = 16
                cText.RichText = true
                cText.TextColor3 = Color3.fromRGB(180, 180, 180)
                cText.TextXAlignment = Enum.TextXAlignment.Center
                cText.TextTruncate = Enum.TextTruncate.AtEnd
                
                cText.Text = "<b><font color='rgb(255,255,255)'>" .. (cmd.NAME or "Unknown") .. "</font></b> - <font color='rgb(120,120,120)'>" .. (cmd.DESC or "") .. "</font>"
            end
            ListFrame.CanvasSize = UDim2.new(0, 0, 0, (#allCmds * 35))
        end

        Background.Visible = true
        Utilities.tween(Background, "Quart", "Out", 0.4, {BackgroundTransparency = Visualizer.Options.Transparency})
    else
        local t = Utilities.tween(Background, "Quart", "In", 0.3, {BackgroundTransparency = 1})
        task.spawn(function()
            t.Completed:Wait()
            Background.Visible = false
        end)
...
Modules['Telekinesis'] = (function()
local Telekinesis = {}

function Telekinesis.Run()
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    ...
    Tool.Grip = CFrame.new(0, 0, 1, 1, 0, 0, 0, 1, 0, 0, 0, 1)
    ...
    linePart.BrickColor = BrickColor.new("Institutional white")
    local lineMesh = Instance.new("BlockMesh", linePart)
    ...
    Tool.Equipped:Connect(function(mouse)
        ...
        if getgenv().notify then
            getgenv().notify("Telekinesis Controls", "Q: -5 Dist | E: +5 Dist\nU: Bank 45Â° | P: Vol Up\nL: Lock Rot\nY: Dist 100 | J: Dist 5000\nX: Dist 15", math.huge)
        end

        selectionBox.Parent = LocalPlayer.Character

        local function onKeyDown(key)
            key = string.lower(key)
            if key == "q" and dist >= 5 then dist = dist - 5 end
            if key == "e" then dist = dist + 5 end
            if key == "y" then dist = 100 end
            if key == "j" then dist = 5000 end
            if key == "x" then dist = 15 end
            
            if targetObject then
                if key == "u" then
                    local bx = Instance.new("BodyGyro")
                    bx.MaxTorque = Vector3.new(math.huge, 0, math.huge)
                    bx.CFrame = bx.CFrame * CFrame.Angles(0, math.rad(45), 0)
                    bx.D = 0
                    bx.Parent = targetObject
                elseif key == "p" then
                    local bv = Instance.new("BodyVelocity")
                    bv.MaxForce = Vector3.new(0, math.huge, 0)
                    bv.Velocity = Vector3.new(0, 1, 0)
                    bv.Parent = targetObject
                elseif key == "l" then
                    for _, v in pairs(targetObject:GetChildren()) do
                        if v:IsA("BodyGyro") then return end
                    end
                    local bg = Instance.new("BodyGyro")
                    bg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
                    bg.CFrame = CFrame.new(targetObject.Position)
                    bg.Parent = targetObject
        ...
        mouse.Button1Down:Connect(function()
            ...
            if mouse.Target and not mouse.Target.Anchored then
                ...
                selectionBox.Adornee = targetObject
                dist = (targetObject.Position - Handle.Position).Magnitude
                
                targetBP = Instance.new("BodyPosition")
                targetBP.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                targetBP.P = targetBP.P * 3
                targetBP.Parent = targetObject
            end

            linePart.Parent = workspace
            renderConn = RunService.RenderStepped:Connect(function()
                local tPos = mouse.Hit.p
                if targetObject and targetBP then
                    tPos = targetObject.Position
                    local lv = CFrame.new(Handle.Position, mouse.Hit.p).LookVector
                    targetBP.Position = Handle.Position + (lv * dist)
                end
                
                local distLine = (Handle.Position - tPos).Magnitude
                linePart.CFrame = CFrame.new(Handle.Position, tPos) * CFrame.new(0, 0, -distLine/2)
                lineMesh.Scale = Vector3.new(0.25, 0.25, distLine)
            end)
        end)
    ...
    if getgenv().notify then 
        getgenv().notify("Telekinesis", "Dropped in Backpack!", 3)
    end
end
...
local Visualizer = require("Visualizer")
local CmdsWindow = require("CmdsWindow")

getgenv().notify = function(title, text, time)
    if getgenv().IY_LEGACY_MODE and notify_legacy then
        notify_legacy(title, text, time)
    else
        Visualizer.Notify(title, text, time)
    end
end

-- Run the animated loading screen
LoadingScreen.Show()

-- Stub a 'Core' object for the Visualizer to use
local CoreStub = {}
CoreStub.ExecuteCommand = function(cmd)
    if execCmd then
        execCmd(cmd, Utilities.LocalPlayer)
    else
        task.spawn(function()
            repeat task.wait() until execCmd
            execCmd(cmd, Utilities.LocalPlayer)
        end)
    end
end
CoreStub.GetCommandSuggestions = function(query)
    ...
    if #args > 1 and args[1] ~= "" then
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if string.find(string.lower(plr.Name), searchName, 1, true) or string.find(string.lower(plr.DisplayName), searchName, 1, true) then
                local pre = ""
                for i=1,#args-1 do pre = pre .. args[i] .. " " end
                table.insert(matches, {Name = pre .. plr.Name, Description = "Player: " .. plr.DisplayName})
            end
        end
    else
        if CMDs then
            for _, cmd in ipairs(CMDs) do
                if string.find(string.lower(cmd.NAME), query, 1, true) then
                    table.insert(matches, {Name = cmd.NAME, Description = cmd.DESC})
                end
            end
        end
    end
...
CoreStub.GetAllCommands = function()
    return CMDs or {}
end

Visualizer.Init(CoreStub)
CmdsWindow.Init(CoreStub)

task.wait(1.5)
LoadingScreen.Hide()
-- suppressed legacy load check
...
Holder.Position = UDim2.new(1.5, 0, 1.5, 0); Holder.Visible = false
...
Title.TextSize = 18
Title.Text = "Infinite Yiff FE v" .. currentVersion
...
pinNotification = nil
local notifyCount = 0
function notify_legacy(text,text2,length)
	task.spawn(function()
...
IYMouse.KeyDown:Connect(function(Key)
	if (Key==prefix) then
		if getgenv().IY_LEGACY_MODE then Cmdbar:CaptureFocus() end
		spawn(function()
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Infinite Yiff support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'legacymode', DESC = 'Toggles the legacy UI and hides Visualizer'}
CMDs[#CMDs + 1] = {NAME = 'credits', DESC = 'Shows Infinite Yiff credits'}
...
addcmd('console',{},function(args, speaker)
	...
	local _, str = pcall(function()
		return game:HttpGet("https://raw.githubusercontent.com/DarkNetworks/Infinite-Yiff/main/console.lua", true)
	end)
...
addcmd('explorer', {'dex'}, function(args, speaker)
    notify('Loading', 'Hold on a sec')
    loadstring(game:HttpGet("https://raw.githubusercontent.com/DarkNetworks/Infinite-Yiff/main/dex.lua"))()
end)
...
addcmd('remotespy',{'rspy'},function(args, speaker)
	...
	loadstring(game:HttpGet("https://raw.githubusercontent.com/DarkNetworks/Infinite-Yiff/main/SimpleSpyV3/main.lua"))()
end)

addcmd('audiologger',{'alogger'},function(args, speaker)
	notify("Loading",'Hold on a sec')
	loadstring(game:HttpGet(('https://raw.githubusercontent.com/DarkNetworks/Infinite-Yiff/main/audiologger.lua'),true))()
end)
...
addcmd('vr',{},function(args, speaker)
	...
	notify("Loading", "Hold on a sec")
	loadstring(game:HttpGet("https://raw.githubusercontent.com/DarkNetworks/Infinite-Yiff/main/vr.lua"))()
end)
...
task.spawn(function()
	...
	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify('Welcome','Welcome to Infinite Yiff ' .. latestVersionInfo.Version)
		end
...
task.spawn(function()
	...
	minimizeHolder()
	if table.find({Enum.Platform.IOS, Enum.Platform.Android}, UserInputService:GetPlatform()) then notify("Unstable Device", "On mobile, Infinite Yiff may have issues or features that are not functioning correctly.") end
end)


-- Inject Custom Legacy and Credits commands
...
execCmd = function(cmd, speaker)
    ...
    elseif args[1] == 'cmds' then
        require("CmdsWindow").Toggle(true)
        return
    elseif args[1] == 'legacymode' then
        if Holder then
            Holder.Visible = not Holder.Visible
            getgenv().IY_LEGACY_MODE = Holder.Visible
            if Holder.Visible then
                Holder.Position = UDim2.new(1, -250, 1, -220)
                Visualizer.Toggle(false)
            else
                Holder.Position = UDim2.new(1.5, 0, 1.5, 0)
            end
        end
        return
    elseif args[1] == 'credits' then
        if getgenv().notify then
            getgenv().notify('Credits', 'Made by ethhereum', 5)
        end
        return
    elseif args[1] == 'vpos' then
        if args[2] == 'top' then
            Visualizer.Options.Position = UDim2.new(0.5, -200, 0, 50)
            Visualizer.Toggle(true)
        elseif args[2] == 'middle' then
            Visualizer.Options.Position = UDim2.new(0.5, -200, 0.5, -20)
            Visualizer.Toggle(true)
        else
            Visualizer.Options.Position = UDim2.new(0.5, -200, 1, -100)
            Visualizer.Toggle(true)
        end
        return
    end
    if OriginalExec then
